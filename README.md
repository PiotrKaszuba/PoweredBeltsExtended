...
- If underground belt ends from a matched pair are powered by separate electric networks and if one of the networks is low on power and would turn off their undeground end while the other end remains powered; it will result in alternating powered and unpowered states of the underground pair with the duration of each state determined by the the internal order of belt entities held by the mod and the amount of ticks it takes to loop through all entities.
